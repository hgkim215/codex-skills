...
handoff_summary="$(printf '%s\n' "$output" | awk -F= '/^WORKER_HANDOFF_SUMMARY=/{print $2; exit}')"
persistent_handoff_dir="$(printf '%s\n' "$output" | awk -F= '/^PERSISTENT_HANDOFF_DIR=/{print $2; exit}')"
persistent_handoff_summary="$(printf '%s\n' "$output" | awk -F= '/^PERSISTENT_HANDOFF_SUMMARY=/{print $2; exit}')"
persistent_handoff_index="$(printf '%s\n' "$output" | awk -F= '/^PERSISTENT_HANDOFF_INDEX=/{print $2; exit}')"
[[ -n "$run_dir" && -d "$run_dir" ]] || fail "RUN_DIR was not produced"
[[ -n "$handoff_summary" ]] || fail "WORKER_HANDOFF_SUMMARY was not produced"
[[ -n "$persistent_handoff_dir" ]] || fail "PERSISTENT_HANDOFF_DIR was not produced"
[[ -n "$persistent_handoff_summary" ]] || fail "PERSISTENT_HANDOFF_SUMMARY was not produced"
[[ -n "$persistent_handoff_index" ]] || fail "PERSISTENT_HANDOFF_INDEX was not produced"
...
[[ -f "$handoff_summary" ]] || fail "worker handoff summary missing"
grep -q 'Ralph Worker Handoff Summary' "$handoff_summary" || fail "worker handoff summary missing title"
grep -q 'Workers At A Glance' "$handoff_summary" || fail "worker handoff summary missing overview"
grep -q 'Final Handoff' "$handoff_summary" || fail "worker handoff summary missing final handoff section"
grep -q 'E2E_STATUS: ok' "$handoff_summary" || fail "worker handoff summary missing final worker content"
[[ -d "$persistent_handoff_dir" ]] || fail "persistent handoff dir missing"
[[ -f "$persistent_handoff_summary" ]] || fail "persistent worker handoff summary missing"
[[ -f "$persistent_handoff_dir/run_summary.md" ]] || fail "persistent run summary missing"
[[ -f "$persistent_handoff_dir/workers.tsv" ]] || fail "persistent workers.tsv missing"
[[ -f "$persistent_handoff_dir/last_messages/worker1.md" ]] || fail "persistent worker last message missing"
[[ -f "$persistent_handoff_index" ]] || fail "persistent handoff index missing"
grep -q 'ralph-release-e2e' "$persistent_handoff_index" || fail "persistent handoff index missing run name"
...
printf '%s\n' "$watch_output" | grep -q 'worker_handoff_summary: present' || fail "watch output missing worker handoff summary"
...
printf 'E2E_HANDOFF_SUMMARY=%s\n' "$handoff_summary"
printf 'E2E_PERSISTENT_HANDOFF_SUMMARY=%s\n' "$persistent_handoff_summary"
